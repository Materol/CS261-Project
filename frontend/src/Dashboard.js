    ...
    if (props.fetchProjects == true) {
      axiosInstance.get("/projects/" + props.email).then((res) => {
        ...
        function myFunction(item) {
          ...
          const overall = (
            (item.currentMetric.overall_success / 5) *
            100
          ).toFixed(1);
          tempProject = {
            id: item.id,
                  ...
                  return (
                    <Carousel.Item key={project.id}>
                      <Row
                        ...
                        xs={1}
                        md={index + 1 < projects.length ? 2 : 1}
                      >
                        <Col md="auto">
                            ...
                            <Card.Body>
                              ...
                              <Row md={2}>
                                <Col>
                                  <CircularProgressbar
                                    styles={buildStyles({
                                      pathColor: percentToColor(
                                        project.successChance
                                      ),
                                      textColor: "black",
                                    })}
                                    value={project.successChance}
                                    text={`${project.successChance}%`}
                                  />
                                  <p className="text-center m-0 fs-10">
                                    <i>Success Chance</i>
                                  </p>
                                </Col>
                                <Col>
                                ...
                                <Row md={2}>
                                  <Col>
                                    <CircularProgressbar
                                      styles={buildStyles({
                                        pathColor: percentToColor(
                                          projects[index + 1].successChance
                                        ),
                                        textColor: "black",
                                      })}
